    ...
    <>
      ...
      <main className="mx-auto w-full max-w-screen-lg flex-1 px-4 py-6">
        ...
        >
          ...
          <Field label="대표 이미지">
            {editing?.thumbnail_url && (
              <>
                ...
                <img src={editing.thumbnail_url} alt="" className="mb-2 h-20 w-20 rounded-lg object-cover" />
                <input type="hidden" name="existing_thumbnail" value={editing.thumbnail_url} />
              </>
            )}
            <input type="file" name="image" accept="image/*" className={input} />
            <span className="mt-1 block text-xs text-gray-400">
              휴대폰/PC 사진 선택{editing ? " (새로 선택 안 하면 기존 이미지 유지)" : ""}
            </span>
          </Field>
          ...
          <div className="sm:col-span-2">
            <span className="mb-1 block text-xs font-medium text-gray-600">노출 매장 (선택 안 하면 전체 매장)</span>
            {stores.length === 0 ? (
              <p className="text-sm text-gray-400">등록된 매장이 없습니다.</p>
            ) : (
                ...
                {stores.map((s) => (
                  <label key={s.id} className="flex items-center gap-1.5 text-sm">
                    <input
                      ...
                      name="store_ids"
                      value={s.id}
                      defaultChecked={editing?.store_ids?.includes(s.id) ?? false}
                      className="h-4 w-4"
                    /> {s.name}
                  </label>
                ))}
          ...
          <div className="flex gap-2 sm:col-span-2">
            ...
            {editing && <Link href="/admin/products" prefetch={false} className="rounded-xl border border-gray-300 px-5 py-2.5 font-medium">취소</Link>}
          </div>
        </form>
            ...
            <div key={p.id} className="flex items-center justify-between rounded-2xl border border-gray-200 bg-white p-3 text-sm">
              <div>
                <p className="font-medium">
                  {p.name}
                  {p.is_visible ? (
                    <span className="ml-2 rounded bg-emerald-100 px-1.5 py-0.5 text-xs text-emerald-700">노출중</span>
                  ) : (
                    <span className="ml-2 rounded bg-gray-100 px-1.5 py-0.5 text-xs text-gray-500">숨김</span>
                  )}
